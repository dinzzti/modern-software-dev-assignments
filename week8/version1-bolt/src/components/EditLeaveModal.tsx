...
interface EditLeaveModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (id: string, request: Partial<LeaveRequest>) => void;
...
export function EditLeaveModal({ isOpen, onClose, onUpdate, request }: EditLeaveModalProps) {
  const [formData, setFormData] = useState({
    employee_name: '',
    leave_type: 'Cuti Tahunan' as LeaveType,
    start_date: '',
    end_date: '',
    reason: '',
  });
          ...
          >
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between rounded-t-2xl">
                <h2 className="text-2xl font-bold text-slate-900">Edit Leave Request</h2>
                <button
                  onClick={handleClose}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <div>
                  <label htmlFor="edit_employee_name" className="block text-sm font-semibold text-slate-700 mb-2">
                    Employee Name
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor="edit_leave_type" className="block text-sm font-semibold text-slate-700 mb-2">
                    Leave Type
                  </label>
                  <select
                    id="edit_leave_type"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="edit_start_date" className="block text-sm font-semibold text-slate-700 mb-2">
                      Start Date
                    </label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label htmlFor="edit_end_date" className="block text-sm font-semibold text-slate-700 mb-2">
                      End Date
                    </label>
                    <input
                      type="date"
                ...
                <div>
                  <label htmlFor="edit_reason" className="block text-sm font-semibold text-slate-700 mb-2">
                    Reason
                  </label>
                  <textarea
                    id="edit_reason"
