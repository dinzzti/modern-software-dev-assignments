...
I will give you a function that failed some unit tests, along with the error messages.

Your goal is to REWRITE the function to fix the bugs.
1. Analyze the error messages to see what is missing (e.g., missing uppercase, missing special char).
2. Output ONLY the fixed Python code block.
"""
...
def your_build_reflexion_context(prev_code: str, failures: List[str]) -> str:
    """Fungsi ini menggabungkan kodingan yang salah + daftar error
    untuk dikirim kembali ke AI agar diperbaiki.
    """
    # Gabungkan semua pesan error jadi satu teks
    error_report = "\n".join(failures)

    # Susun pesan lengkap untuk AI
    return f"""
    ...
    Please fix the code so it passes all validation checks.
    """
